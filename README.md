...
- 🎯 **13+ 个独立 Skill**：可按需单独使用，也可一键输出完整投资报告
...
| `stock-report-builder` | 生成标准格式投资分析报告（10章） | 股票代码 | `core/report.py` |
...
| `shareholder-analyzer` | 十大流通股东/国家队/机构持股/增减持变动 | 股票代码 | `core/shareholder.py` |
...
skill(name="stock-report-builder")  # 生成标准报告
...
# 生成标准格式投资报告（10章）
python .opencode/skills/stock-report-builder/scripts/main.py 600519
...
python .opencode/skills/pdf-converter/scripts/main.py "file.pdf"
```
...
根据《投资分析报告模板.md》生成标准格式投资分析报告（10章结构）。

**报告章节**：
1. 公司概况（基本信息、核心资产布局）
2. 近期行情分析（今日行情、股价表现）
3. 财务分析（主要财务数据、资产负债表、现金流量）
4. 业务板块分析（核心业务、战略储备）
5. 估值分析（PE/PB、业绩预测、分红历史）
6. 风险因素（5类风险）
7. 股东结构分析（十大股东、控股股东、增减持）
8. 综合评分（6维度）
9. 投资建议（短/中/长期、目标价位）
10. 结论（优势、劣势、一句话总结）
...
|----------|----------|------|------|----------|
| 600338 | 西藏珠峰 | 66/120 | B 良好 | `output/西藏珠峰_600338_综合投资分析报告_2026-04-24.md` |
| 000895 | 双汇发展 | 79/120 | B 良好 | `output/双汇发展_000895_综合投资分析报告_2026-04-24.md` |

> 报告文件保存在 `output/` 目录下，可通过邮件发送
