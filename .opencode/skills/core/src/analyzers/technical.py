...
class TechnicalAnalyzer:
    """股票技术指标分析（MA 均线系统 + RSI 动量指标）"""

    def get_historical_data(
        self, stock_code: str, start_date: str = "20240101", end_date: str = None
    ) -> pd.DataFrame:
        """
        获取股票历史 K 线数据。
    ...
    def calculate_ma(
        self, df: pd.DataFrame, short_period: int = 50, long_period: int = 200
    ) -> dict:
        """
        计算移动平均线并判断金叉/死叉。

        Returns:
            {ma50, ma200, signal, note}
            数据不足时回退到 {ma_short, ma_long, signal, note}
        """
