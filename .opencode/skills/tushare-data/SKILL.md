---
name: tushare-data
description: Tushare数据研究工具。面向中文自然语言，将口语化查询转为数据获取、清洗、对比、筛选与导出。覆盖A股行情、财务、估值、资金流、板块概念等场景
author: tushare.pro
version: 1.1.10
credentials:
  - name: TUSHARE_TOKEN
...
## AI评估要求（注：本技能为数据检索，不适用常规评估）

tushare-data 是数据检索skill，不是分析skill，返回的是原始数据表而非评估结果。

当使用本技能时，AI应自行对返回的数据进行以下基础校验：

### 1. 数据完整性校验
- **输入数据**: 数据表
- **校验项**:
  - 行数是否合理（空表可能因日期/未上市/无权限）
  - 必填字段是否存在
  - 是否有明显缺失值
- **处理**: 空表/异常提示用户，不直接判定"好/坏"
...
```json
{
  ...
  "metadata": {
    ...
    "请求参数": {...},
    "数据行数": 100,
    "字段列表": ["ts_code", "trade_date", "open", "high", "low", "close", "vol"],
    "时间范围": "20240101-20240322"
  }
}
```

**注意**: 本skill只返回原始数据和数据质量校验结果，不包含价值判断。AI需根据用户任务自行解读数据。
